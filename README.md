...
- **[freebreix/local-openai-tts-server](https://github.com/freebreix/local-openai-tts-server)** — a fork of [`ginto-sakata/local-openai-tts-server`](https://github.com/ginto-sakata/local-openai-tts-server) (Piper), patched for Windows wheels + 24 kHz PCM streaming + a language-routing `auto` voice.
...
| voice per session | same default for everyone | **hashed to one stable voice** from a pool, per Claude session |
| Windows | WSL / Linux installers | native: Piper + whisper.cpp CUDA, one `install.ps1` |
| language | one | `en` + `hu`, auto-detected per utterance, routed to the right voice |
...
```
 ...
 Claude session C ─┘                        │        voice = hash(session id) % pool
                                            │
   shared file lock  ~/.voicemode/conch  ◄──┘   (turn-taking across the separate processes)
                                            │
              ┌─────────────────────────────┴───────────────┐
        TTS :8880  (Piper fork, scheduled task)      STT :2022  (whisper.cpp CUDA, scheduled task)
```
...
All in the managed block of `~/.voicemode/voicemode.env`:

| var | default | meaning |
|---|---|---|
| `VOICEMODE_STANDUP_VOICES` | `amy-medium,ryan-medium,lessac-medium,hfc_female-medium,kristin-medium` | pool; each session hashes onto one |
...
update.ps1           re-pull pinned upstreams
manifest.psd1        pinned versions
...
tts/        (gitignored)  clone of the TTS fork @ tag + its venv + models
